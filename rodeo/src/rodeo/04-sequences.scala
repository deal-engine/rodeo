...
object Sequences extends Chapter {
  /*
     ...
     We can a have other different implementations of a Sequence, like a Vector or an Array that have more specific optimizations
     for allocating chunks of memory than a linked list, but we will  use them only as as needed.
   */
  ...
  // The head method is an "unsafe" operation in emtpy lists since we can't obtain the first element of an empty sequence.
  ...
  // To check if a given list contains a certain element we use the Boolean valued `contains` method.
  ...
  Exercise("Prepend, append, concatenate") {
    ...
    /*
       In Scala we can concatenate two Sequences using the ++ method. We can also add an element to an existing
       Sequence; at the begging(end) by use of the +:(:+) method.
     */
    ...
    val moreAndMoreAndMoreAndMoreNumbers = moreAndMoreAndMoreNumbers :+ 6
    assertTrue(moreAndMoreAndMoreAndMoreNumbers == ???)
  }
  ...
  Exercise("forall") {
    ...
    val result = numbers.forall(x => {
      x % 3 == 0
    })
    val vacuity = empty.forall(x => {
      x > x
    })
    assertTrue(result != ???)
    assertTrue(vacuity == ???)
  ...
  Exercise("find") {
    ...
    val resultOption = boringnumbers.find(x => {
      x % 3 == 0
    })
    val resultNone = boringnumbers.find(x => {
      x % 5 == 0
    })
  ...
  Exercise("exists") {
    val boringnumbers: Seq[Int] = Seq(1, 2, 4, 6, 8, 9)
    val exists_result = boringnumbers.exists(x => {
      x % 3 == 0 && x % 2 == 0
    })
